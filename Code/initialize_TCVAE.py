...
#         tc_loss = K.log(q_score / (1 - q_score)) 
        ...
        # v1 = z[:,0][:,tf.newaxis]
        # v2 = z[::-1,1][:,tf.newaxis]
        # v3 = tf.keras.layers.concatenate([z[0::2,2],z[1::2,2]], axis=0)[:,tf.newaxis]
        # v4 = tf.keras.layers.concatenate([z[1::2,3],z[0::2,3]], axis=0)[:,tf.newaxis]
        # z_shuffled = tf.keras.layers.concatenate([v1,v2,v3,v4],axis=1)

        col = 0
        rand = np.random.permutation(10)
        indices = tf.constant([[rand[0], 0], [rand[1], col],[rand[2], col],[rand[3], col],[rand[4], col],[rand[5], col],[rand[6], col],[rand[7], col],[rand[8], col],[rand[9], col]])
        v1=tf.gather_nd(z,indices=indices)[:,tf.newaxis]
        
        col = 1
        rand = np.random.permutation(10)
        indices = tf.constant([[rand[0], 0], [rand[1], col],[rand[2], col],[rand[3], col],[rand[4], col],[rand[5], col],[rand[6], col],[rand[7], col],[rand[8], col],[rand[9], col]])
        v2=tf.gather_nd(z,indices=indices)[:,tf.newaxis]
        
        col = 2
        rand = np.random.permutation(10)
        indices = tf.constant([[rand[0], 0], [rand[1], col],[rand[2], col],[rand[3], col],[rand[4], col],[rand[5], col],[rand[6], col],[rand[7], col],[rand[8], col],[rand[9], col]])
        v3=tf.gather_nd(z,indices=indices)[:,tf.newaxis]
        
        col = 3
        rand = np.random.permutation(10)
        indices = tf.constant([[rand[0], 0], [rand[1], col],[rand[2], col],[rand[3], col],[rand[4], col],[rand[5], col],[rand[6], col],[rand[7], col],[rand[8], col],[rand[9], col]])
        v4=tf.gather_nd(z,indices=indices)[:,tf.newaxis]
        
        z_shuffled = tf.concat([v1,v2,v3,v4],axis=1)
        #print(z_shuffled)

        q_bar_score = (discriminator(z_shuffled)+.1) *.85 # +.1 * .85 so that it's 0<x<1
        q_score = (discriminator(z)+.1) *.85 
        
        # q_bar_score = discriminator(z_shuffled)
        # q_score = discriminator(z)        
        # tc_loss = K.log(q_score / (1 - q_score)) 
